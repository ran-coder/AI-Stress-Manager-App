...
| **Manual Prompt Check-Ins** | * **Zero False Positives:** Intervention relies entirely on explicit user confirmation, ensuring support is only delivered when genuinely wanted.<br>* **High User Agency:** Preserves full user control and privacy without relying on continuous background tracking. | * **High Cognitive Load:** Demands executive function and self-awareness from a student who is already overwhelmed.<br>* **Reactive Delay:** Fails to support students who passively avoid tasks or freeze instead of actively seeking help. |
| **System Activity Monitoring** | * **Proactive Detection:** Continuously identifies silent task avoidance, procrastination, and paralysis without waiting for manual input.<br>* **Zero User Friction:** Operates quietly in the background, removing the effort required to initiate help. | * **False Alarm Risk:** May misidentify normal breaks or non-linear study habits as task overwhelm if thresholds are poorly tuned.<br>* **Perceived Intrusiveness:** Can feel micromanaged or invasive if the automated prompts trigger too frequently. |
